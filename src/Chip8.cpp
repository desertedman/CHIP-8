...
uint8_t SDL_KEYS[Chip8::NUM_KEYS]{
    ...
    SDLK_1, SDLK_2, SDLK_3, SDLK_q, SDLK_w, SDLK_e, SDLK_a, SDLK_s, SDLK_d, SDLK_z, SDLK_c, SDLK_4, SDLK_r, SDLK_f,
...
void Chip8::loadRom(const std::string &path)
{
    ...
    std::memset(mCPU.mMemory.data() + Constants::MEMORY_START, 0, Rom::ROM_FILE_SIZE);

    // Read rom data into memory
    newRom.mFile.read(reinterpret_cast<char *>(mCPU.mMemory.data() + Constants::MEMORY_START), newRom.getSize());
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        else if (event.type == SDL_KEYDOWN)
        {
            ...
            else
            {
                ...
                {
                    if (event.key.keysym.sym == SDL_KEYS[i] && mCPU.mInternalKeys[i] == false)
                    {
